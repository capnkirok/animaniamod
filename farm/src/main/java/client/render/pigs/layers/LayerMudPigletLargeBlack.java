...
import client.render.pigs.RenderPigletLargeBlack;
...
public class LayerMudPigletLargeBlack implements LayerRenderer<PigEntityletLargeBlack>
{
	...
	@Override
	public void doRenderLayer(PigEntityletLargeBlack LivingEntityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
	{
		...
		if (LivingEntityIn.getMuddy())
		{
			...
			if (splashTimer <= 0)
			{
				...
				this.pigRenderer.bindTexture(LayerMudPigletLargeBlack.TEXTURE);
