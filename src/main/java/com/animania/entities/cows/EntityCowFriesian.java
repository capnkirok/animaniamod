...
public class EntityCowFriesian extends EntityAnimal
{
	...
	public boolean processInteract(EntityPlayer player, EnumHand hand)
	{
		...
		if (this.getFed() && this.getWatered() && stack != ItemStack.EMPTY && stack.getItem() == Items.BUCKET)
		{
			...
			if (stack.getCount() == 0)
			{
				player.setHeldItem(hand, new ItemStack(Animania.milkBucketFriesian));
			}
			else if (!player.inventory.addItemStackToInventory(new ItemStack(Animania.milkBucketFriesian)))
			{
				player.dropItem(new ItemStack(Animania.milkBucketFriesian), false);
			}
