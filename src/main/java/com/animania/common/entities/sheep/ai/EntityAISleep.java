	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			if (entityIn.world.isDaytime()) {
				if (entityIn instanceof EntityAnimaniaSheep) {
			...
			String bedBlockString = AnimaniaConfig.careAndFeeding.sheepBed;
			ItemStack bedBlockStack = AnimaniaHelper.getItem(bedBlockString);
			Block bedBlock = Block.getBlockFromItem(bedBlockStack.getItem());

			String bedBlockString2 = AnimaniaConfig.careAndFeeding.sheepBed2;
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaSheep && poschk == bedBlock) {
					EntityAnimaniaSheep ech = (EntityAnimaniaSheep)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaSheep && poschk == bedBlock2) {
					EntityAnimaniaSheep ech = (EntityAnimaniaSheep)entityIn;
			...
			for (int i = -16; i < 16; i++) {
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						List<EntityAnimaniaSheep> others = AnimaniaHelper.getEntitiesInRange(EntityAnimaniaSheep.class, 2, entityIn.world, pos);
