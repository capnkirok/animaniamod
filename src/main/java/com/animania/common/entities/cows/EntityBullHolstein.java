...
public class EntityBullHolstein extends EntityCow
{

    private static final DataParameter<Optional<UUID>> MATE_UNIQUE_ID   = EntityDataManager.<Optional<UUID>> createKey(EntityBullHolstein.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Boolean>        FIGHTING         = EntityDataManager.<Boolean> createKey(EntityBullHolstein.class, DataSerializers.BOOLEAN);
    private static final Set<Item>                     TEMPTATION_ITEMS = Sets.newHashSet(new Item[] { Items.WHEAT });
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityBullHolstein.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityBullHolstein.class, DataSerializers.BOOLEAN);
    ...
    public EntityBullHolstein(World world) {
        ...
        this.tasks.addTask(7, new EntityAITempt(this, 1.25D, false, EntityBullHolstein.TEMPTATION_ITEMS));
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityBullHolstein.MATE_UNIQUE_ID, Optional.<UUID> absent());
        this.dataManager.register(EntityBullHolstein.FIGHTING, Boolean.valueOf(false));
        this.dataManager.register(EntityBullHolstein.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityBullHolstein.WATERED, Boolean.valueOf(true));
    }
    ...
    protected void consumeItemFromStack(EntityPlayer player, ItemStack stack) {
        ...
        this.eatTimer = 80;
        player.addStat(AnimaniaAchievements.Holstein, 1);
    ...
    public static void registerFixesCow(DataFixer fixer) {
        EntityLiving.registerFixesMob(fixer, EntityBullHolstein.class);
    }
    ...
    public UUID getMateUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityBullHolstein.MATE_UNIQUE_ID)).orNull();
    }

    public void setMateUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityBullHolstein.MATE_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFighting() {
        return this.dataManager.get(EntityBullHolstein.FIGHTING).booleanValue();
    }

    public void setFighting(boolean fighting) {
        if (fighting)
            this.dataManager.set(EntityBullHolstein.FIGHTING, Boolean.valueOf(true));
        else
            this.dataManager.set(EntityBullHolstein.FIGHTING, Boolean.valueOf(false));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityBullHolstein.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityBullHolstein.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityBullHolstein.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityBullHolstein.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityBullHolstein.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityBullHolstein.WATERED, Boolean.valueOf(false));
    }
    ...
    @Override
    public EntityBullHolstein createChild(EntityAgeable p_90011_1_) {
        return null;
    }
