...
{
	private final EntityAnimaniaCow entityIn;
	...
	public EntityAIFindFood(EntityAnimaniaCow entityInIn, double speedIn)
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings) {
			...
			if (poschk == BlockHandler.blockTrough || poschk1 == BlockHandler.blockTrough || poschk2 == BlockHandler.blockTrough || poschk3 == BlockHandler.blockTrough || poschk4 == BlockHandler.blockTrough || poschk5 == BlockHandler.blockTrough || poschk6 == BlockHandler.blockTrough || poschk7 == BlockHandler.blockTrough || poschk8 == BlockHandler.blockTrough) {
				...
				if (te != null && te.canConsume(EntityAnimaniaCow.TEMPTATION_ITEMS, null)) {
					...
					if (entityIn instanceof EntityAnimaniaCow) {
				...
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						if (blockchk == BlockHandler.blockTrough) {
							...
							if (te != null && te.canConsume(EntityAnimaniaCow.TEMPTATION_ITEMS, null) && entityIn.getNavigator().tryMoveToXYZ(pos.getX(), pos.getY(), pos.getZ(), 1.0)) {
								foodFound = true;
						...
						if (blockchk == Blocks.RED_FLOWER || blockchk instanceof BlockCrops|| blockchk == Blocks.WHEAT || blockchk == Blocks.YELLOW_FLOWER) {
							...
							if (newloc < loc) {
								...
								if (entityIn.posX < foodPos.getX()) {
									BlockPos foodPoschk = new BlockPos(x + i + 1, y + j, z + k);
									Block foodBlockchk = entityIn.world.getBlockState(foodPoschk).getBlock();
									if (foodBlockchk == Blocks.RED_FLOWER || foodBlockchk instanceof BlockCrops|| foodBlockchk == Blocks.WHEAT || foodBlockchk == Blocks.YELLOW_FLOWER) {
										i = i + 1;
									}
								} 
