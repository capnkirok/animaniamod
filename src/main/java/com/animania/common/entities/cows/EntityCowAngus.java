...
public class EntityCowAngus extends EntityCow
{
    ...
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityCowAngus.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityCowAngus.class,
            DataSerializers.BOOLEAN);
    ...
    public EntityCowAngus(World world) {
        ...
        this.tasks.addTask(4, new EntityAIPanicCows(this, 2.5D));
        this.tasks.addTask(5, new EntityAIMateCows(this, 1.0D));
        this.tasks.addTask(6, new EntityAITempt(this, 1.25D, false, EntityCowAngus.TEMPTATION_ITEMS));
    ...
    public static void registerFixesCow(DataFixer fixer) {
        EntityLiving.registerFixesMob(fixer, EntityCowAngus.class);
    }
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityCowAngus.MATE_UNIQUE_ID, Optional.<UUID> absent());
        this.dataManager.register(EntityCowAngus.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityCowAngus.WATERED, Boolean.valueOf(true));
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if (cowCount <= AnimaniaConfig.spawn.spawnLimitCows) {
            ...
            if (chooser == 0) {
                EntityBullAngus entityCow = new EntityBullAngus(this.world);
                ...
                EntityCalfAngus entityCow = new EntityCalfAngus(this.world);
                entityCow.setPosition(this.posX, this.posY, this.posZ);
                this.world.spawnEntity(entityCow);
    ...
    public UUID getMateUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityCowAngus.MATE_UNIQUE_ID)).orNull();
    }

    public void setMateUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityCowAngus.MATE_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityCowAngus.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityCowAngus.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityCowAngus.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityCowAngus.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityCowAngus.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityCowAngus.WATERED, Boolean.valueOf(false));
    }
    ...
    public void onLivingUpdate() {
        ...
        // System.out.println(this.getMateUniqueId());
    ...
    protected void dropFewItems(boolean hit, int lootlevel) {
		...
		} else
		{
            ...
            dropItem = new ItemStack(ItemHandler.rawAngusBeef);
            if (this.isBurning())
                dropItem = new ItemStack(ItemHandler.cookedAngusRoast);
        }
