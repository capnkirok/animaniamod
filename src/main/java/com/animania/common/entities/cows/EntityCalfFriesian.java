...
public class EntityCalfFriesian extends EntityCow
{
    private static final DataParameter<Optional<UUID>> PARENT_UNIQUE_ID = EntityDataManager.<Optional<UUID>> createKey(EntityCalfFriesian.class,
            DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityCalfFriesian.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityCalfFriesian.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Float>          AGE              = EntityDataManager.<Float> createKey(EntityCalfFriesian.class,
    ...
    public EntityCalfFriesian(World world) {
        ...
        this.tasks.addTask(4, new EntityAITempt(this, 1.25D, false, EntityCalfFriesian.TEMPTATION_ITEMS));
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityCalfFriesian.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfFriesian.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfFriesian.AGE, Float.valueOf(0));
        this.dataManager.register(EntityCalfFriesian.PARENT_UNIQUE_ID, Optional.<UUID> absent());
    }
    ...
    protected void consumeItemFromStack(EntityPlayer player, ItemStack stack) {
        ...
        if (player.hasAchievement(AnimaniaAchievements.Friesian) && player.hasAchievement(AnimaniaAchievements.Friesian)
    ...
    public UUID getParentUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityCalfFriesian.PARENT_UNIQUE_ID)).orNull();
    }

    public void setParentUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityCalfFriesian.PARENT_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityCalfFriesian.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityCalfFriesian.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityCalfFriesian.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityCalfFriesian.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityCalfFriesian.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityCalfFriesian.WATERED, Boolean.valueOf(false));
    }

    public float getEntityAge() {
        return this.dataManager.get(EntityCalfFriesian.AGE).floatValue();
    }

    public void setEntityAge(float age) {
        this.dataManager.set(EntityCalfFriesian.AGE, Float.valueOf(age));
    }
