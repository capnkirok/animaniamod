...
public class EntityCalfHereford extends EntityAnimal
{
    private static final DataParameter<Optional<UUID>> PARENT_UNIQUE_ID = EntityDataManager.<Optional<UUID>> createKey(EntityCalfHereford.class,
            DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityCalfHereford.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityCalfHereford.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Float>          AGE              = EntityDataManager.<Float> createKey(EntityCalfHereford.class,
    ...
    public EntityCalfHereford(World world) {
        ...
        this.tasks.addTask(4, new EntityAITempt(this, 1.25D, false, EntityCalfHereford.TEMPTATION_ITEMS));
    ...
    public static void registerFixesCow(DataFixer fixer) {
        EntityLiving.registerFixesMob(fixer, EntityCalfHereford.class);
    }
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityCalfHereford.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfHereford.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfHereford.AGE, Float.valueOf(0));
        this.dataManager.register(EntityCalfHereford.PARENT_UNIQUE_ID, Optional.<UUID> absent());
    }
    ...
    protected void consumeItemFromStack(EntityPlayer player, ItemStack stack) {
        ...
        if (player.hasAchievement(AnimaniaAchievements.Hereford) && player.hasAchievement(AnimaniaAchievements.Friesian)
    ...
    public UUID getParentUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityCalfHereford.PARENT_UNIQUE_ID)).orNull();
    }

    public void setParentUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityCalfHereford.PARENT_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityCalfHereford.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityCalfHereford.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityCalfHereford.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityCalfHereford.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityCalfHereford.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityCalfHereford.WATERED, Boolean.valueOf(false));
    }

    public float getEntityAge() {
        return this.dataManager.get(EntityCalfHereford.AGE).floatValue();
    }

    public void setEntityAge(float age) {
        this.dataManager.set(EntityCalfHereford.AGE, Float.valueOf(age));
    }
            ...
            if (fed && watered) {
                ...
                if (age >= 1.0 && !this.world.isRemote) {
                    ...
                    if (this.rand.nextInt(2) < 1) {
                        EntityCowHereford entityCow = new EntityCowHereford(this.world);
                        ...
                        EntityBullHereford entityBull = new EntityBullHereford(this.world);
    ...
    @Override
    public EntityCalfHereford createChild(EntityAgeable p_90011_1_) {
        return null;
    }
}