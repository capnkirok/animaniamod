...
public class EntityCalfLonghorn extends EntityCow
{
    private static final DataParameter<Optional<UUID>> PARENT_UNIQUE_ID = EntityDataManager.<Optional<UUID>> createKey(EntityCalfLonghorn.class,
    ...
    public EntityCalfLonghorn(World world) {
        ...
        this.tasks.addTask(4, new EntityAITempt(this, 1.25D, false, EntityCalfLonghorn.TEMPTATION_ITEMS));
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityCalfLonghorn.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfLonghorn.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfLonghorn.AGE, Float.valueOf(0));
        this.dataManager.register(EntityCalfLonghorn.PARENT_UNIQUE_ID, Optional.<UUID> absent());
    }
    ...
    public UUID getParentUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityCalfLonghorn.PARENT_UNIQUE_ID)).orNull();
    }

    public void setParentUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityCalfLonghorn.PARENT_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityCalfLonghorn.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityCalfLonghorn.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityCalfLonghorn.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityCalfLonghorn.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityCalfLonghorn.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityCalfLonghorn.WATERED, Boolean.valueOf(false));
    }

    public float getEntityAge() {
        return this.dataManager.get(EntityCalfLonghorn.AGE).floatValue();
    }

    public void setEntityAge(float age) {
        this.dataManager.set(EntityCalfLonghorn.AGE, Float.valueOf(age));
    }
