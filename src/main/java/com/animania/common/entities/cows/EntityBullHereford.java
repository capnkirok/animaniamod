...
public class EntityBullHereford extends EntityCow
{

	private static final DataParameter<Optional<UUID>> MATE_UNIQUE_ID   = EntityDataManager.<Optional<UUID>> createKey(EntityBullHereford.class,
			DataSerializers.OPTIONAL_UNIQUE_ID);
	private static final DataParameter<Boolean>        FIGHTING         = EntityDataManager.<Boolean> createKey(EntityBullHereford.class,
			DataSerializers.BOOLEAN);
	private static final Set<Item>                     TEMPTATION_ITEMS = Sets.newHashSet(new Item[] { Items.WHEAT });
	private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityBullHereford.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityBullHereford.class,
			DataSerializers.BOOLEAN);
	...
	public EntityBullHereford(World world) {
		...
		this.tasks.addTask(7, new EntityAITempt(this, 1.25D, false, EntityBullHereford.TEMPTATION_ITEMS));
	...
	protected void entityInit() {
		super.entityInit();
		this.dataManager.register(EntityBullHereford.MATE_UNIQUE_ID, Optional.<UUID> absent());
	...
	public static void registerFixesCow(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, EntityBullHereford.class);
	}
	...
	public UUID getMateUniqueId() {
		return (UUID) ((Optional) this.dataManager.get(EntityBullHereford.MATE_UNIQUE_ID)).orNull();
	}

	public void setMateUniqueId(@Nullable UUID uniqueId) {
		this.dataManager.set(EntityBullHereford.MATE_UNIQUE_ID, Optional.fromNullable(uniqueId));
	}

	public boolean getFighting() {
		return this.dataManager.get(EntityBullHereford.FIGHTING).booleanValue();
	}

	public void setFighting(boolean fighting) {
		if (fighting)
			this.dataManager.set(EntityBullHereford.FIGHTING, Boolean.valueOf(true));
		else
			this.dataManager.set(EntityBullHereford.FIGHTING, Boolean.valueOf(false));
	}

	public boolean getFed() {
		return this.dataManager.get(EntityBullHereford.FED).booleanValue();
	}

	public void setFed(boolean fed) {
		if (fed) {
			this.dataManager.set(EntityBullHereford.FED, Boolean.valueOf(true));
			this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
			this.setHealth(this.getHealth() + 1.0F);
		}
		else
			this.dataManager.set(EntityBullHereford.FED, Boolean.valueOf(false));
	}

	public boolean getWatered() {
		return this.dataManager.get(EntityBullHereford.WATERED).booleanValue();
	}

	public void setWatered(boolean watered) {
		if (watered) {
			this.dataManager.set(EntityBullHereford.WATERED, Boolean.valueOf(true));
			this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
		}
		else
			this.dataManager.set(EntityBullHereford.WATERED, Boolean.valueOf(false));
	}
	...
	protected void dropFewItems(boolean hit, int lootlevel) {
		...
		{
			dropItem = new ItemStack(ItemHandler.rawHerefordBeef);
			if (this.isBurning())
				dropItem = new ItemStack(ItemHandler.cookedHerefordRoast);
		}
	...
	@Override
	public EntityBullHereford createChild(EntityAgeable p_90011_1_) {
		return null;
	}
}