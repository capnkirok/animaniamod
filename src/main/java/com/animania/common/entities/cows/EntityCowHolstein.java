...
public class EntityCowHolstein extends EntityAnimal
{
    private static final DataParameter<Optional<UUID>> MATE_UNIQUE_ID   = EntityDataManager.<Optional<UUID>> createKey(EntityCowHolstein.class,
            DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityCowHolstein.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityCowHolstein.class,
            DataSerializers.BOOLEAN);
    private static final Set<Item>                     TEMPTATION_ITEMS = Sets.newHashSet(new Item[] { Items.WHEAT });
    private int                                        happyTimer;
    private int                                        gestationTimer;
    public int                                         blinkTimer;
    private ItemStack                                  milkHolstein;
    ...
    public EntityCowHolstein(World world) {
        ...
        this.tasks.addTask(6, new EntityAITempt(this, 1.25D, false, EntityCowHolstein.TEMPTATION_ITEMS));
        ...
        this.milkHolstein = UniversalBucket.getFilledBucket(ForgeModContainer.getInstance().universalBucket, BlockHandler.fluidMilkHolstein);
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityCowHolstein.MATE_UNIQUE_ID, Optional.<UUID> absent());
        this.dataManager.register(EntityCowHolstein.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityCowHolstein.WATERED, Boolean.valueOf(true));
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if (cowCount <= AnimaniaConfig.spawn.spawnLimitCows) {
            ...
            if (chooser == 0) {
                EntityBullHolstein entityCow = new EntityBullHolstein(this.world);
                ...
                EntityCalfHolstein entityCow = new EntityCalfHolstein(this.world);
                entityCow.setPosition(this.posX, this.posY, this.posZ);
                this.world.spawnEntity(entityCow);
    ...
    public UUID getMateUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityCowHolstein.MATE_UNIQUE_ID)).orNull();
    }

    public void setMateUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityCowHolstein.MATE_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityCowHolstein.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityCowHolstein.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityCowHolstein.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityCowHolstein.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityCowHolstein.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityCowHolstein.WATERED, Boolean.valueOf(false));
    }
                ...
                for (int k = 0; k <= esize - 1; k++) {
                    ...
                    if (entity != null && this.getFed() && this.getWatered() && entity.getPersistentID().toString().equals(MateID) && x2 <= 10
                        ...
                        if (!this.world.isRemote) {
                            ...
                            if (entity instanceof EntityBullHolstein) {
                                EntityCalfHolstein entityCalf = new EntityCalfHolstein(this.world);
    ...
    @Override
    public EntityCowHolstein createChild(EntityAgeable p_90011_1_) {
        return null;
    }
}