...
public class EntityCalfHolstein extends EntityAnimal
{
    private static final DataParameter<Optional<UUID>> PARENT_UNIQUE_ID = EntityDataManager.<Optional<UUID>> createKey(EntityCalfHolstein.class,
            DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityCalfHolstein.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityCalfHolstein.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Float>          AGE              = EntityDataManager.<Float> createKey(EntityCalfHolstein.class,
    ...
    public EntityCalfHolstein(World world) {
        ...
        this.tasks.addTask(4, new EntityAITempt(this, 1.25D, false, EntityCalfHolstein.TEMPTATION_ITEMS));
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityCalfHolstein.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfHolstein.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityCalfHolstein.AGE, Float.valueOf(0));
        this.dataManager.register(EntityCalfHolstein.PARENT_UNIQUE_ID, Optional.<UUID> absent());
    }
    ...
    protected void consumeItemFromStack(EntityPlayer player, ItemStack stack) {
        ...
        if (player.hasAchievement(AnimaniaAchievements.Holstein) && player.hasAchievement(AnimaniaAchievements.Friesian)
    ...
    public UUID getParentUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityCalfHolstein.PARENT_UNIQUE_ID)).orNull();
    }

    public void setParentUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityCalfHolstein.PARENT_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityCalfHolstein.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityCalfHolstein.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityCalfHolstein.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityCalfHolstein.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityCalfHolstein.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityCalfHolstein.WATERED, Boolean.valueOf(false));
    }

    public float getEntityAge() {
        return this.dataManager.get(EntityCalfHolstein.AGE).floatValue();
    }

    public void setEntityAge(float age) {
        this.dataManager.set(EntityCalfHolstein.AGE, Float.valueOf(age));
    }
            ...
            if (fed && watered) {
                ...
                if (age >= 1.0 && !this.world.isRemote) {
                    ...
                    if (this.rand.nextInt(2) < 1) {
                        EntityCowHolstein entityCow = new EntityCowHolstein(this.world);
                        ...
                        EntityBullHolstein entityBull = new EntityBullHolstein(this.world);
    ...
    @Override
    public EntityCalfHolstein createChild(EntityAgeable p_90011_1_) {
        return null;
    }
}