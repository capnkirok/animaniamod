...
{
    private static final DataParameter<String>  COLOR            = EntityDataManager.<String> createKey(EntityHenOrpington.class, DataSerializers.STRING);
    private static final DataParameter<Boolean> FED              = EntityDataManager.<Boolean> createKey(EntityHenOrpington.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> WATERED          = EntityDataManager.<Boolean> createKey(EntityHenOrpington.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> LAID             = EntityDataManager.<Boolean> createKey(EntityHenOrpington.class, DataSerializers.BOOLEAN);
    ...
    public EntityHenOrpington(World world) {
        ...
        this.tasks.addTask(7, new EntityAITempt(this, 1.2D, false, EntityHenOrpington.TEMPTATION_ITEMS));
        ...
        this.laidTimer = AnimaniaConfig.careAndFeeding.laidTimer / 2 + 1500 + this.rand.nextInt(100);
        this.happyTimer = 60;
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if (chickenCount <= AnimaniaConfig.spawn.spawnLimitChickens) {
            ...
            if (chooser == 0) {
                EntityRoosterOrpington entityChicken = new EntityRoosterOrpington(this.world);
                entityChicken.setPosition(this.posX, this.posY, this.posZ);
                this.world.spawnEntity(entityChicken);
            }
            else if (chooser == 1) {
                EntityChickOrpington entityChicken = new EntityChickOrpington(this.world);
                entityChicken.setPosition(this.posX, this.posY, this.posZ);
                this.world.spawnEntity(entityChicken);
            }
            else if (chooser > 2) {
                ...
                entityChicken.setPosition(this.posX, this.posY, this.posZ);
                this.world.spawnEntity(entityChicken);
                EntityChickOrpington entityChick = new EntityChickOrpington(this.world);
    ...
    public String getColor() {
        return this.dataManager.get(EntityHenOrpington.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityHenOrpington.COLOR, String.valueOf(color));
    }
    ...
    private String getRandomColor() {
        return "animania:textures/entity/chickens/hen_golden.png";
    }

    private String getTextureBlink() {
        return "animania:textures/entity/chickens/hen_golden_blink.png";
    }

    private void ColorInitialize() {
        if (EntityHenOrpington.ColorList == null) {
            EntityHenOrpington.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityHenOrpington.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityHenOrpington.COLOR, "");
        this.dataManager.register(EntityHenOrpington.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityHenOrpington.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityHenOrpington.LAID, Boolean.valueOf(true));
    }
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityHenOrpington.ColorList.contains(s1)) {
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityHenOrpington.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityHenOrpington.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityHenOrpington.FED, Boolean.valueOf(false));
    }

    public boolean getLaid() {
        return this.dataManager.get(EntityHenOrpington.LAID).booleanValue();
    }

    public void setLaid(boolean laid) {
        if (laid) {
            this.dataManager.set(EntityHenOrpington.LAID, Boolean.valueOf(true));
            this.laidTimer = AnimaniaConfig.careAndFeeding.laidTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityHenOrpington.LAID, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityHenOrpington.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityHenOrpington.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityHenOrpington.WATERED, Boolean.valueOf(false));
    }
    ...
    protected void dropFewItems(boolean hit, int lootlevel) {
        ...
        if (AnimaniaConfig.drops.customMobDrops) {
            ...
            dropItem = new ItemStack(ItemHandler.rawOrpingtonChicken, 1);
            if (this.isBurning())
                dropItem = new ItemStack(ItemHandler.cookedOrpingtonChicken, 1);
        }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityHenOrpington.TEMPTATION_ITEMS.contains(stack.getItem());
    }
