...
public class EntityRoosterWyandotte extends EntityChicken
{
    private static final DataParameter<String>  COLOR            = EntityDataManager.<String> createKey(EntityRoosterWyandotte.class,
            DataSerializers.STRING);
    private static final DataParameter<Integer> CROWTIMER        = EntityDataManager.<Integer> createKey(EntityRoosterWyandotte.class,
            DataSerializers.VARINT);
    private static final DataParameter<Integer> CROWDURATION     = EntityDataManager.<Integer> createKey(EntityRoosterWyandotte.class,
            DataSerializers.VARINT);
    private static final DataParameter<Boolean> FED              = EntityDataManager.<Boolean> createKey(EntityRoosterWyandotte.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> WATERED          = EntityDataManager.<Boolean> createKey(EntityRoosterWyandotte.class,
            DataSerializers.BOOLEAN);
    ...
    public EntityRoosterWyandotte(World world) {
        ...
        this.tasks.addTask(2, new EntityAITempt(this, 1.2D, false, EntityRoosterWyandotte.TEMPTATION_ITEMS));
    ...
    public String getColor() {
        return this.dataManager.get(EntityRoosterWyandotte.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityRoosterWyandotte.COLOR, String.valueOf(color));
    }
    ...
    private String getRandomColor() {
        return "animania:textures/entity/chickens/rooster_brown.png";
    }

    private String getTextureBlink() {
        return "animania:textures/entity/chickens/rooster_brown_blink.png";
    }

    private void ColorInitialize() {
        if (EntityRoosterWyandotte.ColorList == null) {
            EntityRoosterWyandotte.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityRoosterWyandotte.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityRoosterWyandotte.COLOR, "");
        this.dataManager.register(EntityRoosterWyandotte.CROWTIMER, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterWyandotte.CROWDURATION, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterWyandotte.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityRoosterWyandotte.WATERED, Boolean.valueOf(true));
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityRoosterWyandotte.ColorList.contains(s1)) {
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityRoosterWyandotte.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityRoosterWyandotte.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityRoosterWyandotte.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityRoosterWyandotte.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityRoosterWyandotte.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityRoosterWyandotte.WATERED, Boolean.valueOf(false));
    }

    public int getCrowDuration() {
        return this.dataManager.get(EntityRoosterWyandotte.CROWDURATION).intValue();
    }

    public void setCrowDuration(int duration) {
        this.dataManager.set(EntityRoosterWyandotte.CROWDURATION, Integer.valueOf(duration));
    }

    public int getTimeUntilNextCrow() {
        return this.dataManager.get(EntityRoosterWyandotte.CROWTIMER).intValue();
    }

    public void setTimeUntilNextCrow(int timer) {
        this.dataManager.set(EntityRoosterWyandotte.CROWTIMER, Integer.valueOf(timer));
    }
    ...
    protected void dropFewItems(boolean hit, int lootlevel) {
        ...
        if (AnimaniaConfig.drops.customMobDrops) {
            ...
            dropItem = new ItemStack(ItemHandler.rawWyandotteChicken);
            if (this.isBurning())
                dropItem = new ItemStack(ItemHandler.cookedWyandotteChicken);
        }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityRoosterWyandotte.TEMPTATION_ITEMS.contains(stack.getItem());
    }
    ...
    @Override
    public EntityRoosterWyandotte createChild(EntityAgeable ageable) {
