...
public class EntityAIFindFood extends EntityAIBase 
{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= 60) {
			...
			if (temptedEntity instanceof EntityAnimaniaChicken) {
				EntityAnimaniaChicken ech = (EntityAnimaniaChicken)temptedEntity;
				if (ech.getFed()) {
					this.delayTemptCounter = 0;
					return false;		
				}
	...
	public void startExecuting()
	{
		...
		for (int i = -16; i < 16; i++) {
			for (int j = -3; j < 3; j++) {
				for (int k = -16; k < 16; k++) {
					...
					} else if (blockchk == BlockHandler.blockSeeds) {
						foodFound = true;
						newloc = Math.abs(i)  +  Math.abs(j) +  Math.abs(k);
		...
		if (foodFound) {
			...
			if (mudBlockchk == BlockHandler.blockSeeds || mudBlockchk == BlockHandler.blockTrough) {
