...
public class EntityRoosterLeghorn extends EntityAnimal
{
    ...
    public EntityRoosterLeghorn(World world) {
        ...
        this.tasks.addTask(2, new EntityAITempt(this, 1.2D, false, EntityRoosterLeghorn.TEMPTATION_ITEMS));
    ...
    public String getColor() {
        return this.dataManager.get(EntityRoosterLeghorn.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityRoosterLeghorn.COLOR, String.valueOf(color));
    }
    ...
    private void ColorInitialize() {
        if (EntityRoosterLeghorn.ColorList == null) {
            EntityRoosterLeghorn.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityRoosterLeghorn.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityRoosterLeghorn.COLOR, "");
        this.dataManager.register(EntityRoosterLeghorn.CROWTIMER, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterLeghorn.CROWDURATION, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterLeghorn.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityRoosterLeghorn.WATERED, Boolean.valueOf(true));
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityRoosterLeghorn.ColorList.contains(s1)) {
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityRoosterLeghorn.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityRoosterLeghorn.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityRoosterLeghorn.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityRoosterLeghorn.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityRoosterLeghorn.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityRoosterLeghorn.WATERED, Boolean.valueOf(false));
    }

    public int getCrowDuration() {
        return this.dataManager.get(EntityRoosterLeghorn.CROWDURATION).intValue();
    }

    public void setCrowDuration(int duration) {
        this.dataManager.set(EntityRoosterLeghorn.CROWDURATION, Integer.valueOf(duration));
    }

    public int getTimeUntilNextCrow() {
        return this.dataManager.get(EntityRoosterLeghorn.CROWTIMER).intValue();
    }

    public void setTimeUntilNextCrow(int timer) {
        this.dataManager.set(EntityRoosterLeghorn.CROWTIMER, Integer.valueOf(timer));
    }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityRoosterLeghorn.TEMPTATION_ITEMS.contains(stack.getItem());
    }
