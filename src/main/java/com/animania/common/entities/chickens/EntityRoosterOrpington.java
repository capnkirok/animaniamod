...
public class EntityRoosterOrpington extends EntityChicken
{
    ...
    public String getColor() {
        return this.dataManager.get(EntityRoosterOrpington.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityRoosterOrpington.COLOR, String.valueOf(color));
    }
    ...
    private void ColorInitialize() {
        if (EntityRoosterOrpington.ColorList == null) {
            EntityRoosterOrpington.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityRoosterOrpington.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityRoosterOrpington.COLOR, "");
        this.dataManager.register(EntityRoosterOrpington.CROWTIMER, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterOrpington.CROWDURATION, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterOrpington.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityRoosterOrpington.WATERED, Boolean.valueOf(true));
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityRoosterOrpington.ColorList.contains(s1)) {
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityRoosterOrpington.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityRoosterOrpington.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityRoosterOrpington.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityRoosterOrpington.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityRoosterOrpington.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityRoosterOrpington.WATERED, Boolean.valueOf(false));
    }

    public int getCrowDuration() {
        return this.dataManager.get(EntityRoosterOrpington.CROWDURATION).intValue();
    }

    public void setCrowDuration(int duration) {
        this.dataManager.set(EntityRoosterOrpington.CROWDURATION, Integer.valueOf(duration));
    }

    public int getTimeUntilNextCrow() {
        return this.dataManager.get(EntityRoosterOrpington.CROWTIMER).intValue();
    }

    public void setTimeUntilNextCrow(int timer) {
        this.dataManager.set(EntityRoosterOrpington.CROWTIMER, Integer.valueOf(timer));
    }
    ...
    protected void dropFewItems(boolean hit, int lootlevel) {
        ...
        if (AnimaniaConfig.drops.customMobDrops) {
            ...
            dropItem = new ItemStack(ItemHandler.rawOrpingtonChicken);
            if (this.isBurning())
                dropItem = new ItemStack(ItemHandler.cookedOrpingtonChicken);
        }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityRoosterOrpington.TEMPTATION_ITEMS.contains(stack.getItem());
    }
    ...
    @Override
    public EntityRoosterOrpington createChild(EntityAgeable ageable) {
