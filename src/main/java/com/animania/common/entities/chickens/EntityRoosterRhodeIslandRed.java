...
public class EntityRoosterRhodeIslandRed extends EntityChicken
{
    private static final DataParameter<String>  COLOR            = EntityDataManager.<String> createKey(EntityRoosterRhodeIslandRed.class, DataSerializers.STRING);
    private static final DataParameter<Integer> CROWTIMER        = EntityDataManager.<Integer> createKey(EntityRoosterRhodeIslandRed.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> CROWDURATION     = EntityDataManager.<Integer> createKey(EntityRoosterRhodeIslandRed.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> FED              = EntityDataManager.<Boolean> createKey(EntityRoosterRhodeIslandRed.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> WATERED          = EntityDataManager.<Boolean> createKey(EntityRoosterRhodeIslandRed.class, DataSerializers.BOOLEAN);
    ...
    public EntityRoosterRhodeIslandRed(World world) {
        ...
        this.tasks.addTask(2, new EntityAITempt(this, 1.2D, false, EntityRoosterRhodeIslandRed.TEMPTATION_ITEMS));
    ...
    public String getColor() {
        return this.dataManager.get(EntityRoosterRhodeIslandRed.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityRoosterRhodeIslandRed.COLOR, String.valueOf(color));
    }
    ...
    private void ColorInitialize() {
        if (EntityRoosterRhodeIslandRed.ColorList == null) {
            EntityRoosterRhodeIslandRed.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityRoosterRhodeIslandRed.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityRoosterRhodeIslandRed.COLOR, "");
        this.dataManager.register(EntityRoosterRhodeIslandRed.CROWTIMER, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterRhodeIslandRed.CROWDURATION, Integer.valueOf(0));
        this.dataManager.register(EntityRoosterRhodeIslandRed.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityRoosterRhodeIslandRed.WATERED, Boolean.valueOf(true));
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityRoosterRhodeIslandRed.ColorList.contains(s1)) {
    ...
    public void onLivingUpdate() {
        ...
        if ((currentTime > 23250 || currentTime < 500) && this.getTimeUntilNextCrow() == 0) {
            ...
            if (crowChooser == 0)
                this.world.playSound(null, this.posX, this.posY, this.posZ, ModSoundEvents.chickenCrow1, SoundCategory.PLAYERS, 0.7F,
                        0.95F + modular);
            else if (crowChooser == 1)
                this.world.playSound(null, this.posX, this.posY, this.posZ, ModSoundEvents.chickenCrow2, SoundCategory.PLAYERS, 0.65F,
                        0.9F + modular);
            else if (crowChooser == 2)
                this.world.playSound(null, this.posX, this.posY, this.posZ, ModSoundEvents.chickenCrow3, SoundCategory.PLAYERS, 0.6F,
                        1.05F + modular);
            this.setTimeUntilNextCrow(this.rand.nextInt(200) + 200);
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityRoosterRhodeIslandRed.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityRoosterRhodeIslandRed.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityRoosterRhodeIslandRed.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityRoosterRhodeIslandRed.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityRoosterRhodeIslandRed.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityRoosterRhodeIslandRed.WATERED, Boolean.valueOf(false));
    }

    public int getCrowDuration() {
        return this.dataManager.get(EntityRoosterRhodeIslandRed.CROWDURATION).intValue();
    }

    public void setCrowDuration(int duration) {
        this.dataManager.set(EntityRoosterRhodeIslandRed.CROWDURATION, Integer.valueOf(duration));
    }

    public int getTimeUntilNextCrow() {
        return this.dataManager.get(EntityRoosterRhodeIslandRed.CROWTIMER).intValue();
    }

    public void setTimeUntilNextCrow(int timer) {
        this.dataManager.set(EntityRoosterRhodeIslandRed.CROWTIMER, Integer.valueOf(timer));
    }
    ...
    protected void dropFewItems(boolean hit, int lootlevel) {
        ...
        if (AnimaniaConfig.drops.customMobDrops) {
            ...
            dropItem = new ItemStack(ItemHandler.rawRhodeIslandRedChicken);
            if (this.isBurning())
                dropItem = new ItemStack(ItemHandler.cookedRhodeIslandRedChicken);
        }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityRoosterRhodeIslandRed.TEMPTATION_ITEMS.contains(stack.getItem());
    }
    ...
    @Override
    public EntityRoosterRhodeIslandRed createChild(EntityAgeable ageable) {
