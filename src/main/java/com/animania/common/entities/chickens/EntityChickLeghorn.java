...
{
    private static final DataParameter<String>  COLOR            = EntityDataManager.<String> createKey(EntityChickLeghorn.class, DataSerializers.STRING);
    private static final DataParameter<Boolean> FED              = EntityDataManager.<Boolean> createKey(EntityChickLeghorn.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> WATERED          = EntityDataManager.<Boolean> createKey(EntityChickLeghorn.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Float>   AGE              = EntityDataManager.<Float> createKey(EntityChickLeghorn.class,  DataSerializers.FLOAT);
    ...
    public EntityChickLeghorn(World world) {
        ...
        this.tasks.addTask(4, new EntityAITempt(this, 1.2D, false, EntityChickLeghorn.TEMPTATION_ITEMS));
    ...
    public String getColor() {
        return this.dataManager.get(EntityChickLeghorn.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityChickLeghorn.COLOR, String.valueOf(color));
    }
    ...
    private String getRandomColor() {
        return "animania:textures/entity/chickens/chick_white.png";
    }

    private String getTextureBlink() {
        return "animania:textures/entity/chickens/chick_white_blink.png";
    }

    private void ColorInitialize() {
        if (EntityChickLeghorn.ColorList == null) {
            EntityChickLeghorn.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityChickLeghorn.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityChickLeghorn.COLOR, "");
        this.dataManager.register(EntityChickLeghorn.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityChickLeghorn.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityChickLeghorn.AGE, Float.valueOf(0));
    }
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityChickLeghorn.ColorList.contains(s1)) {
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityChickLeghorn.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityChickLeghorn.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityChickLeghorn.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityChickLeghorn.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityChickLeghorn.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityChickLeghorn.WATERED, Boolean.valueOf(false));
    }

    public float getEntityAge() {
        return this.dataManager.get(EntityChickLeghorn.AGE).floatValue();
    }

    public void setEntityAge(float age) {
        this.dataManager.set(EntityChickLeghorn.AGE, Float.valueOf(age));
    }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityChickLeghorn.TEMPTATION_ITEMS.contains(stack.getItem());
    }
