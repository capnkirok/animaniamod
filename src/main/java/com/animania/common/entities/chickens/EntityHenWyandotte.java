...
public class EntityHenWyandotte extends EntityChicken
{
	private static final DataParameter<String>  COLOR            = EntityDataManager.<String> createKey(EntityHenWyandotte.class, DataSerializers.STRING);
	private static final DataParameter<Boolean> FED              = EntityDataManager.<Boolean> createKey(EntityHenWyandotte.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> WATERED          = EntityDataManager.<Boolean> createKey(EntityHenWyandotte.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> LAID             = EntityDataManager.<Boolean> createKey(EntityHenWyandotte.class, DataSerializers.BOOLEAN);
	...
	public EntityHenWyandotte(World world) {
		...
		this.tasks.addTask(7, new EntityAITempt(this, 1.2D, false, EntityHenWyandotte.TEMPTATION_ITEMS));
		...
		this.laidTimer = AnimaniaConfig.careAndFeeding.laidTimer / 2 + 1000 + this.rand.nextInt(100);
		this.happyTimer = 60;
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
		...
		if (chickenCount <= AnimaniaConfig.spawn.spawnLimitChickens) {
			...
			if (chooser == 0) {
				EntityRoosterWyandotte entityChicken = new EntityRoosterWyandotte(this.world);
				entityChicken.setPosition(this.posX, this.posY, this.posZ);
				this.world.spawnEntity(entityChicken);
			}
			else if (chooser == 1) {
				EntityChickWyandotte entityChicken = new EntityChickWyandotte(this.world);
				entityChicken.setPosition(this.posX, this.posY, this.posZ);
				this.world.spawnEntity(entityChicken);
			}
			else if (chooser > 2) {
				...
				entityChicken.setPosition(this.posX, this.posY, this.posZ);
				this.world.spawnEntity(entityChicken);
				EntityChickWyandotte entityChick = new EntityChickWyandotte(this.world);
	...
	public String getColor() {
		return this.dataManager.get(EntityHenWyandotte.COLOR);
	}

	public void setColor(String color) {
		this.dataManager.set(EntityHenWyandotte.COLOR, String.valueOf(color));
	}
	...
	private String getRandomColor() {
		return "animania:textures/entity/chickens/hen_brown.png";
	}

	private String getTextureBlink() {
		return "animania:textures/entity/chickens/hen_brown_blink.png";
	}

	private void ColorInitialize() {
		if (EntityHenWyandotte.ColorList == null) {
			EntityHenWyandotte.ColorList = new ArrayList();
			try {
				...
				for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
					...
					if (m.matches()) {
						String s = m.group(1);
						EntityHenWyandotte.ColorList.add(s);
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	protected void entityInit() {
		super.entityInit();
		this.dataManager.register(EntityHenWyandotte.COLOR, "");
		this.dataManager.register(EntityHenWyandotte.FED, Boolean.valueOf(true));
		this.dataManager.register(EntityHenWyandotte.WATERED, Boolean.valueOf(true));
		this.dataManager.register(EntityHenWyandotte.LAID, Boolean.valueOf(true));
	}
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		if (s1.length() > 0)
			if (EntityHenWyandotte.ColorList.contains(s1)) {
	...
	public boolean getFed() {
		return this.dataManager.get(EntityHenWyandotte.FED).booleanValue();
	}

	public void setFed(boolean fed) {
		if (fed) {
			this.dataManager.set(EntityHenWyandotte.FED, Boolean.valueOf(true));
			this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
			this.setHealth(this.getHealth() + 1.0F);
		}
		else
			this.dataManager.set(EntityHenWyandotte.FED, Boolean.valueOf(false));
	}

	public boolean getLaid() {
		return this.dataManager.get(EntityHenWyandotte.LAID).booleanValue();
	}

	public void setLaid(boolean laid) {
		if (laid) {
			this.dataManager.set(EntityHenWyandotte.LAID, Boolean.valueOf(true));
			this.laidTimer = AnimaniaConfig.careAndFeeding.laidTimer + this.rand.nextInt(100);
		}
		else
			this.dataManager.set(EntityHenWyandotte.LAID, Boolean.valueOf(false));
	}

	public boolean getWatered() {
		return this.dataManager.get(EntityHenWyandotte.WATERED).booleanValue();
	}

	public void setWatered(boolean watered) {
		if (watered) {
			this.dataManager.set(EntityHenWyandotte.WATERED, Boolean.valueOf(true));
			this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
		}
		else
			this.dataManager.set(EntityHenWyandotte.WATERED, Boolean.valueOf(false));
	}
	...
	public boolean isBreedingItem(@Nullable ItemStack stack) {
		return stack != ItemStack.EMPTY && EntityHenWyandotte.TEMPTATION_ITEMS.contains(stack.getItem());
	}
	...
	@Override
	public EntityHenWyandotte createChild(EntityAgeable ageable) {
