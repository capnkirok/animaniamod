	...
	public boolean shouldExecute() {
		...
		if (delayTemptCounter > 10) {
			...
			if (poschk == BlockHandler.blockNest) {
				...
				if (te.getNestType() == 3 || te.getNestType() == 6 || te.getNestType() == 9 || te.getNestType() == 12
						|| te.getNestType() == 15) {
					return false;
				}
				...
				if (temptedEntity instanceof EntityHenLeghorn) {
					...
					if (te != null && te.getNestType() == 0 && !entity.getLaid()) {
						...
						te.markDirty();
						entity.setLaid(true);
						this.temptedEntity.world.notifyBlockUpdate(currentpos, poschk.getDefaultState(),
								poschk.getDefaultState(), 1);
	...
	public void updateTask() {
		...
		if (poschk != BlockHandler.blockNest) {
			...
			if (nestFound) {
				...
				List<Entity> nestClear = temptedEntity.world.getEntitiesWithinAABBExcludingEntity(temptedEntity,
						temptedEntity.getEntityBoundingBox().expandXyz(1));

				if (nestBlockchk == BlockHandler.blockNest && nestClear.isEmpty()) {
					this.temptedEntity.getNavigator().tryMoveToXYZ(nestPos.getX() + .50, nestPos.getY(),
							nestPos.getZ() + .50, this.speed);
				} else {
