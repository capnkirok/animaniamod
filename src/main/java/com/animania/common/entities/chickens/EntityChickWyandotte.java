...
public class EntityChickWyandotte extends EntityAnimal
{
    ...
    public EntityChickWyandotte(World world) {
        ...
        this.tasks.addTask(4, new EntityAITempt(this, 1.2D, false, EntityChickWyandotte.TEMPTATION_ITEMS));
        ...
        this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer / 3 + this.rand.nextInt(100);
        this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer / 3 + this.rand.nextInt(100);
        this.happyTimer = 60;
        this.ageTimer = 0;
    ...
    public String getColor() {
        return this.dataManager.get(EntityChickWyandotte.COLOR);
    }

    public void setColor(String color) {
        this.dataManager.set(EntityChickWyandotte.COLOR, String.valueOf(color));
    }
    ...
    private void ColorInitialize() {
        if (EntityChickWyandotte.ColorList == null) {
            EntityChickWyandotte.ColorList = new ArrayList();
            try {
                ...
                for (ResourceInfo i : ClassPath.from(this.getClass().getClassLoader()).getResources()) {
                    ...
                    if (m.matches()) {
                        String s = m.group(1);
                        EntityChickWyandotte.ColorList.add(s);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityChickWyandotte.COLOR, "");
        this.dataManager.register(EntityChickWyandotte.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityChickWyandotte.WATERED, Boolean.valueOf(true));
        this.dataManager.register(EntityChickWyandotte.AGE, Float.valueOf(0));
    }
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
        ...
        if (s1.length() > 0)
            if (EntityChickWyandotte.ColorList.contains(s1)) {
    ...
    public boolean getFed() {
        return this.dataManager.get(EntityChickWyandotte.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityChickWyandotte.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityChickWyandotte.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityChickWyandotte.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityChickWyandotte.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityChickWyandotte.WATERED, Boolean.valueOf(false));
    }

    public float getEntityAge() {
        return this.dataManager.get(EntityChickWyandotte.AGE).floatValue();
    }

    public void setEntityAge(float age) {
        this.dataManager.set(EntityChickWyandotte.AGE, Float.valueOf(age));
    }
    ...
    public boolean isBreedingItem(@Nullable ItemStack stack) {
        return stack != ItemStack.EMPTY && EntityChickWyandotte.TEMPTATION_ITEMS.contains(stack.getItem());
    }
