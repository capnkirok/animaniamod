...
public class EntityAISleep extends EntityAIBase 
{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			if (entityIn instanceof EntityRoosterBase) {
				long currentTime = entityIn.world.getWorldTime() % 23999;
			...
			if (entityIn.world.isDaytime()) {
				if (entityIn instanceof EntityAnimaniaPeacock) {
					EntityAnimaniaPeacock ech = (EntityAnimaniaPeacock)entityIn;
					if (ech.getSleeping()) {
						ech.setSleeping(false);
						this.delayTemptCounter = 0;
			...
			String bedBlockString = AnimaniaConfig.careAndFeeding.peacockBed;
			ItemStack bedBlockStack = AnimaniaHelper.getItem(bedBlockString);
			Block bedBlock = Block.getBlockFromItem(bedBlockStack.getItem());

			String bedBlockString2 = AnimaniaConfig.careAndFeeding.peacockBed2;
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaPeacock && poschk == bedBlock) {
					EntityAnimaniaPeacock ech = (EntityAnimaniaPeacock)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaPeacock && poschk == bedBlock2) {
					EntityAnimaniaPeacock ech = (EntityAnimaniaPeacock)entityIn;
			...
			for (int i = -16; i < 16; i++) {
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						List<EntityAnimaniaPeacock> others = AnimaniaHelper.getEntitiesInRange(EntityAnimaniaPeacock.class, 2, entityIn.world, pos);
						...
						if (!prefBedFound && blockchk == bedBlock2 && others.size() < 2) {
	...
	public void startExecuting()
	{
		...
		String bedBlockString2 = AnimaniaConfig.careAndFeeding.peacockBed2;
		ItemStack bedBlockStack2 = AnimaniaHelper.getItem(bedBlockString2);
		Block bedBlock2 = Block.getBlockFromItem(bedBlockStack.getItem());
		...
		if (bedFound) {
			...
			if ((bedBlockchk == bedBlock || bedBlockchk == bedBlock2) && !this.entityIn.hasPath()) {
				if(this.entityIn.getNavigator().tryMoveToXYZ(bedPos.getX(), bedPos.getY(), bedPos.getZ(), this.speed) == false) {
					this.entityIn.getLookHelper().setLookPosition(bedPos.getX(), bedPos.getY(), bedPos.getZ(), (float)this.yaw, (float)this.pitch);
					this.delayTemptCounter = 0;
					this.resetTask();
				} else {
					this.entityIn.getLookHelper().setLookPosition(bedPos.getX(), bedPos.getY(), bedPos.getZ(), (float)this.yaw, (float)this.pitch);
