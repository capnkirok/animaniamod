...
public class EntityAISleepRabbits extends EntityAIBase 
{
	...
	public EntityAISleepRabbits(EntityCreature entityInIn, double speedIn)
	{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			boolean crepuscularWindow = false;
			if ((currentTime > 21000 && currentTime < 24000) || (currentTime > 11000 && currentTime < 14000)) {
				crepuscularWindow = true;
			} 
			
			if (crepuscularWindow) {
			...
			String bedBlockString = AnimaniaConfig.careAndFeeding.rabbitBed;
			ItemStack bedBlockStack = AnimaniaHelper.getItem(bedBlockString);
			Block bedBlock = Block.getBlockFromItem(bedBlockStack.getItem());

			String bedBlockString2 = AnimaniaConfig.careAndFeeding.rabbitBed2;
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaRabbit && poschk == bedBlock) {
					EntityAnimaniaRabbit ech = (EntityAnimaniaRabbit)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaRabbit && poschk == bedBlock2) {
					EntityAnimaniaRabbit ech = (EntityAnimaniaRabbit)entityIn;
