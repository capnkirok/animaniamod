...
public class EntityAISleepHedgehogs extends EntityAIBase 
{
	...
	public EntityAISleepHedgehogs(EntityCreature entityInIn, double speedIn)
	{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			if (entityIn.getCustomNameTag().equals("Sanic") || entityIn.getCustomNameTag().equals("Sonic")) {
				this.delayTemptCounter = 0;
				return false;
			}
			...
			String bedBlockString = AnimaniaConfig.careAndFeeding.hedgehogBed;
			ItemStack bedBlockStack = AnimaniaHelper.getItem(bedBlockString);
			Block bedBlock = Block.getBlockFromItem(bedBlockStack.getItem());

			String bedBlockString2 = AnimaniaConfig.careAndFeeding.hedgehogBed2;
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityHedgehogBase && poschk == bedBlock) {
					EntityHedgehogBase ech = (EntityHedgehogBase)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityHedgehogBase && poschk == bedBlock2) {
					EntityHedgehogBase ech = (EntityHedgehogBase)entityIn;
			...
			for (int i = -16; i < 16; i++) {
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						List<EntityHedgehogBase> others = AnimaniaHelper.getEntitiesInRange(EntityHedgehogBase.class, 2, entityIn.world, pos);
