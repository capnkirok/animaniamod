...
public class EntityAISleepFerrets extends EntityAIBase 
{
	...
	public EntityAISleepFerrets(EntityCreature entityInIn, double speedIn)
	{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			String bedBlockString = AnimaniaConfig.careAndFeeding.ferretBed;
			ItemStack bedBlockStack = AnimaniaHelper.getItem(bedBlockString);
			Block bedBlock = Block.getBlockFromItem(bedBlockStack.getItem());

			String bedBlockString2 = AnimaniaConfig.careAndFeeding.ferretBed2;
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityFerretBase && poschk == bedBlock) {
					EntityFerretBase ech = (EntityFerretBase)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityFerretBase && poschk == bedBlock2) {
					EntityFerretBase ech = (EntityFerretBase)entityIn;
			...
			for (int i = -16; i < 16; i++) {
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						List<EntityFerretBase> others = AnimaniaHelper.getEntitiesInRange(EntityFerretBase.class, 2, entityIn.world, pos);
