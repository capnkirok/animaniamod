...
public class EntityAIHedgehogFindFood extends EntityAIBase {
	...
	public void updateTask() {
		...
		for (int i = -16; i < 16; i++) {
			for (int j = -3; j < 3; j++) {
				for (int k = -16; k < 16; k++) {
					...
					if (blockchk == BlockHandler.blockNest) {
					...
					} else if (blockchk == Blocks.CARROTS || blockchk == Blocks.BEETROOTS
						...
						if (newloc < loc) {
							...
							if (temptedEntity.posX < foodPos.getX()) {
								BlockPos foodPoschk = new BlockPos(x + i + 1, y + j, z + k);
								Block foodBlockchk = temptedEntity.world.getBlockState(foodPoschk).getBlock();
								if (foodBlockchk == Blocks.CARROTS || foodBlockchk == Blocks.BEETROOTS
		...
		if (foodFound) {
			...
			} else if (foodBlockchk == Blocks.CARROTS || foodBlockchk == Blocks.BEETROOTS
					|| foodBlockchk == Blocks.POTATOES) {
