...
public class EntityAIFollowMateRabbits extends EntityAIBase
{
	...
	public EntityAIFollowMateRabbits(EntityAnimal animal, double speed)
	{
	...
	public boolean shouldExecute() {
		...
		if (this.delayCounter > 60)
			if (this.thisAnimal instanceof EntityRabbitBuckBase) {
				EntityRabbitBuckBase ec = (EntityRabbitBuckBase) this.thisAnimal;
				if (ec.getMateUniqueId() == null)
					return false;
				else {

					List entities = AnimaniaHelper.getEntitiesInRange(EntityRabbitDoeBase.class, 40, this.thisAnimal.world, this.thisAnimal);

					for (int k = 0; k <= entities.size() - 1; k++) {

						EntityRabbitDoeBase entity = (EntityRabbitDoeBase)entities.get(k);
						
						if (entities.get(k) != null && entity.getPersistentID().equals(((EntityRabbitBuckBase) this.thisAnimal).getMateUniqueId())) {
