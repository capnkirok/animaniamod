...
public class EntityAIFindWater extends EntityAIBase 
{
	...
	public EntityAIFindWater(EntityCreature entityInIn, double speedIn)
	{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings) {
			...
			if (entityIn instanceof EntityAnimaniaRabbit) {
				EntityAnimaniaRabbit entityIn2 = (EntityAnimaniaRabbit) entityIn;
				if (entityIn2.getWatered() || entityIn2.getSleeping()) {
					this.delayTemptCounter = 0;
					return false;		
				}
			}
			...
			if (entityIn instanceof EntityHamster) {
				EntityHamster entityIn2 = (EntityHamster) entityIn;
				if (entityIn2.getWatered() || entityIn2.getSleeping()) {
					this.delayTemptCounter = 0;
					return false;		
				}
			...
			if (entityIn instanceof EntityFerretBase) {
				EntityFerretBase entityIn2 = (EntityFerretBase) entityIn;
				if (entityIn2.getWatered() || entityIn2.getSleeping()) {
					this.delayTemptCounter = 0;
					return false;		
				}
			...
			if (entityIn instanceof EntityHedgehogBase) {
				EntityHedgehogBase entityIn2 = (EntityHedgehogBase) entityIn;
				if (entityIn2.getWatered() || entityIn2.getSleeping()) {
					this.delayTemptCounter = 0;
					return false;		
				}
			...
			if (poschk == BlockHandler.blockTrough || poschk1 == BlockHandler.blockTrough || poschk2 == BlockHandler.blockTrough || poschk3 == BlockHandler.blockTrough || poschk4 == BlockHandler.blockTrough || poschk5 == BlockHandler.blockTrough || poschk6 == BlockHandler.blockTrough || poschk7 == BlockHandler.blockTrough || poschk8 == BlockHandler.blockTrough) {
				...
				if (te != null && te.fluidHandler.getFluid() != null && te.fluidHandler.getFluid().getFluid() == FluidRegistry.WATER) {
					...
					if (entityIn instanceof EntityAnimaniaRabbit) {
						...
						ech.setWatered(true);
					} else if (entityIn instanceof EntityHamster) {
						EntityHamster ech = (EntityHamster)entityIn;
						ech.setWatered(true);
					} else if (entityIn instanceof EntityFerretBase) {
						EntityFerretBase ech = (EntityFerretBase)entityIn;
						ech.entityAIEatGrass.startExecuting();
						ech.setWatered(true);
					} else if (entityIn instanceof EntityHedgehogBase) {
						EntityHedgehogBase ech = (EntityHedgehogBase)entityIn;
						ech.entityAIEatGrass.startExecuting();
						ech.setWatered(true);
					} 
			...
			for (int i = -16; i < 16; i++) {
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						List<EntityAnimaniaRabbit> others = AnimaniaHelper.getEntitiesInRange(EntityAnimaniaRabbit.class, 2, entityIn.world, pos);
