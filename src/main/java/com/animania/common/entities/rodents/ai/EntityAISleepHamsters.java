...
public class EntityAISleepHamsters extends EntityAIBase 
{
	...
	public EntityAISleepHamsters(EntityCreature entityInIn, double speedIn)
	{
	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			String bedBlockString = AnimaniaConfig.careAndFeeding.hamsterBed;
			ItemStack bedBlockStack = AnimaniaHelper.getItem(bedBlockString);
			Block bedBlock = Block.getBlockFromItem(bedBlockStack.getItem());

			String bedBlockString2 = AnimaniaConfig.careAndFeeding.hamsterBed2;
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityHamster && poschk == bedBlock) {
					EntityHamster ech = (EntityHamster)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityHamster && poschk == bedBlock2) {
					EntityHamster ech = (EntityHamster)entityIn;
			...
			for (int i = -16; i < 16; i++) {
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						List<EntityHamster> others = AnimaniaHelper.getEntitiesInRange(EntityHamster.class, 2, entityIn.world, pos);
		...
		if (bedFound) {
			...
			if ((bedBlockchk == bedBlock || bedBlockchk == bedBlock2) && !this.entityIn.hasPath()) {
				if(this.entityIn.getNavigator().tryMoveToXYZ(bedPos.getX(), bedPos.getY(), bedPos.getZ(), this.speed) == false) {
					...
					this.entityIn.getLookHelper().setLookPosition(bedPos.getX(), bedPos.getY(), bedPos.getZ(), 0.0F, 0.0F);
					this.entityIn.getNavigator().tryMoveToXYZ(bedPos.getX() + .5, bedPos.getY(), bedPos.getZ() + .5, this.speed);
				}
