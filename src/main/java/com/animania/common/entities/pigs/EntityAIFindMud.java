			...
			for (int i = -10; i < 10; i++) {
				for (int j = -2; j < 2; j++) {
					for (int k = -10; k < 10; k++) {
						...
						if (blockchk != null && (blockchk == Animania.blockMud)) {
							mudFound = true;
			...
			for (int i = -10; i < 10; i++) {
				for (int j = -2; j < 2; j++) {
					for (int k = -10; k < 10; k++) {
						...
						pos = new BlockPos(x + i, y + j, z + k);
						Block blockchk = temptedEntity.world.getBlockState(pos).getBlock();
						if (blockchk == Animania.blockMud && !temptedEntity.hasPath()) {
							mudFound = true;
							...
							if (newloc < loc) {
								...
								if (temptedEntity.posX > mudPos.getX()) {
									...
									if (mudBlockchk == Animania.blockMud) {
										spcFlag = true;
										i = i + 1;
									}
								} 
				...
				if (mudBlockchk == Animania.blockMud) {
					if (temptedEntity instanceof EntitySowYorkshire) {
						...
						if (te.getMuddy() == false) {
							if (te.posX - (double)mudPos.getX() < 0 && te.posZ - (double)mudPos.getZ() < 0) {
						...
						if (te.getMuddy() == false) {
							...
							} else if (te.posX - (double)mudPos.getX() > 0 && te.posZ - (double)mudPos.getZ() > 0) {
								this.temptedEntity.getNavigator().tryMoveToXYZ(mudPos.getX() - 1 , mudPos.getY(), mudPos.getZ() - 1, this.speed);
							} else {
								this.temptedEntity.getNavigator().tryMoveToXYZ(mudPos.getX(), mudPos.getY(), mudPos.getZ(), this.speed);
							}
