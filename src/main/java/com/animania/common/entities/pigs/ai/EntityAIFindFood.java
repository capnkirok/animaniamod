...
public class EntityAIFindFood extends EntityAIBase 
{
	...
	public EntityAIFindFood(EntityAnimaniaPig temptedEntityIn, double speedIn)
	{
		this.entityIn = (EntityAnimaniaPig) temptedEntityIn;
		this.speed = speedIn;
		this.setMutexBits(3);
				...
				for (int j = -3; j < 3; j++) {
					for (int k = -16; k < 16; k++) {
						...
						if (blockchk == BlockHandler.blockTrough) {
							...
							if (te != null && (te.fluidHandler.getFluid() != null && te.fluidHandler.getFluid().getFluid() == BlockHandler.fluidSlop) || te.canConsume(EntityAnimaniaPig.TEMPTATION_ITEMS, null)) {
								foodFound = true;
	...
	public void startExecuting()
	{
						...
						if (te != null && (te.fluidHandler.getFluid() != null && te.fluidHandler.getFluid().getFluid() == BlockHandler.fluidSlop) || te.canConsume(EntityAnimaniaPig.TEMPTATION_ITEMS, null)) {
							...
							if (newloc < loc) {
								...
								if (entityIn.posX < foodPos.getX()) {
									...
									Block foodBlockchk = entityIn.world.getBlockState(foodPoschk).getBlock();
									//i = i + 1;
								} 
						...
						if (blockchk == Blocks.RED_FLOWER || blockchk instanceof BlockCrops || blockchk == Blocks.WHEAT || blockchk == Blocks.YELLOW_FLOWER || blockchk == Blocks.TALLGRASS) {
							...
							if (newloc < loc) {
								...
								if (entityIn.posX < foodPos.getX()) {
									BlockPos foodPoschk = new BlockPos(x + i + 1, y + j, z + k);
									Block foodBlockchk = entityIn.world.getBlockState(foodPoschk).getBlock();
									if (foodBlockchk == Blocks.RED_FLOWER || foodBlockchk instanceof BlockCrops || foodBlockchk == Blocks.WHEAT || foodBlockchk == Blocks.YELLOW_FLOWER || blockchk == Blocks.TALLGRASS) {
										i = i + 1;
									}
								} 
		...
		if (foodFound) {
			...
			if (foodBlockchk == BlockHandler.blockTrough) {
				TileEntityTrough te = (TileEntityTrough) this.entityIn.world.getTileEntity(foodPos);
