	...
	public boolean shouldExecute()
	{
		...
		if (this.delayTemptCounter <= AnimaniaConfig.gameRules.ticksBetweenAIFirings + entityIn.getRNG().nextInt(100)) {
			...
			if (bedBlock != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaPig && poschk == bedBlock) {
					EntityAnimaniaPig ech = (EntityAnimaniaPig)entityIn;
			...
			if (bedBlock2 != Blocks.AIR) {
				...
				if (entityIn instanceof EntityAnimaniaPig && poschk == bedBlock2) {
					EntityAnimaniaPig ech = (EntityAnimaniaPig)entityIn;
