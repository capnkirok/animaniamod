...
public class EntityHogYorkshire extends EntityAnimal
{
	private static final DataParameter<Optional<UUID>> MATE_UNIQUE_ID   = EntityDataManager.<Optional<UUID>> createKey(EntityHogYorkshire.class,
			DataSerializers.OPTIONAL_UNIQUE_ID);
	private static final DataParameter<Boolean>        SADDLED          = EntityDataManager.<Boolean> createKey(EntityHogYorkshire.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean>        MUDDY            = EntityDataManager.<Boolean> createKey(EntityHogYorkshire.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Float>          SPLASHTIMER      = EntityDataManager.<Float> createKey(EntityHogYorkshire.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float>          MUDTIMER         = EntityDataManager.<Float> createKey(EntityHogYorkshire.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityHogYorkshire.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityHogYorkshire.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean>        PLAYED           = EntityDataManager.<Boolean> createKey(EntityHogYorkshire.class,
			DataSerializers.BOOLEAN);
	...
	public EntityHogYorkshire(World worldIn) {
		super(worldIn);
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(10, new EntityAITempt(this, 1.2D, false, EntityHogYorkshire.TEMPTATION_ITEMS));
	...
	protected void entityInit() {
		super.entityInit();
		this.dataManager.register(EntityHogYorkshire.MATE_UNIQUE_ID, Optional.<UUID> absent());
		this.dataManager.register(EntityHogYorkshire.SADDLED, Boolean.valueOf(false));
		this.dataManager.register(EntityHogYorkshire.MUDDY, Boolean.valueOf(false));
		this.dataManager.register(EntityHogYorkshire.MUDTIMER, Float.valueOf(0.0F));
		this.dataManager.register(EntityHogYorkshire.SPLASHTIMER, Float.valueOf(0.0F));
		this.dataManager.register(EntityHogYorkshire.FED, Boolean.valueOf(true));
		this.dataManager.register(EntityHogYorkshire.WATERED, Boolean.valueOf(true));
		this.dataManager.register(EntityHogYorkshire.PLAYED, Boolean.valueOf(true));
	}
	...
	public UUID getMateUniqueId() {
		return (UUID) ((Optional) this.dataManager.get(EntityHogYorkshire.MATE_UNIQUE_ID)).orNull();
	}

	public void setMateUniqueId(@Nullable UUID uniqueId) {
		this.dataManager.set(EntityHogYorkshire.MATE_UNIQUE_ID, Optional.fromNullable(uniqueId));
	}
	...
	public void setSaddled(boolean saddled) {
		if (saddled)
			this.dataManager.set(EntityHogYorkshire.SADDLED, Boolean.valueOf(true));
		else
			this.dataManager.set(EntityHogYorkshire.SADDLED, Boolean.valueOf(false));
	}

	public boolean getFed() {
		return this.dataManager.get(EntityHogYorkshire.FED).booleanValue();
	}

	public void setFed(boolean fed) {
		if (fed) {
			this.dataManager.set(EntityHogYorkshire.FED, Boolean.valueOf(true));
			this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
			this.setHealth(this.getHealth() + 1.0F);
		}
		else
			this.dataManager.set(EntityHogYorkshire.FED, Boolean.valueOf(false));
	}
	...
	public boolean getPlayed() {
		return this.dataManager.get(EntityHogYorkshire.PLAYED).booleanValue();
	}

	public void setPlayed(boolean played) {
		if (played) {
			this.dataManager.set(EntityHogYorkshire.PLAYED, Boolean.valueOf(true));
			this.playedTimer = AnimaniaConfig.careAndFeeding.playTimer + this.rand.nextInt(100);
		}
		else
			this.dataManager.set(EntityHogYorkshire.PLAYED, Boolean.valueOf(false));
	}

	public boolean getWatered() {
		return this.dataManager.get(EntityHogYorkshire.WATERED).booleanValue();
	}

	public void setWatered(boolean watered) {
		if (watered) {
			this.dataManager.set(EntityHogYorkshire.WATERED, Boolean.valueOf(true));
			this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
		}
		else
			this.dataManager.set(EntityHogYorkshire.WATERED, Boolean.valueOf(false));
	}

	public boolean getMuddy() {
		return this.dataManager.get(EntityHogYorkshire.MUDDY).booleanValue();
	}

	public void setMuddy(boolean muddy) {
		if (muddy)
			this.dataManager.set(EntityHogYorkshire.MUDDY, Boolean.valueOf(true));
		else
			this.dataManager.set(EntityHogYorkshire.MUDDY, Boolean.valueOf(false));
	}

	public Float getMudTimer() {
		return this.dataManager.get(EntityHogYorkshire.MUDTIMER).floatValue();
	}

	public void setMudTimer(Float timer) {
		this.dataManager.set(EntityHogYorkshire.MUDTIMER, Float.valueOf(timer));
	}

	public Float getSplashTimer() {
		return this.dataManager.get(EntityHogYorkshire.SPLASHTIMER).floatValue();
	}

	public void setSplashTimer(Float timer) {
		this.dataManager.set(EntityHogYorkshire.SPLASHTIMER, Float.valueOf(timer));
	}
	...
	@Override
	public EntityHogYorkshire createChild(EntityAgeable ageable) {
		return null;
	}
	...
	public boolean isBreedingItem(@Nullable ItemStack stack) {
		return stack != ItemStack.EMPTY
				&& (EntityHogYorkshire.TEMPTATION_ITEMS.contains(stack.getItem()) || ItemStack.areItemStacksEqual(stack, this.slop));
	}
}