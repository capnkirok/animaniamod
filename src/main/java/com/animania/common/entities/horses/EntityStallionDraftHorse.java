...
public class EntityStallionDraftHorse extends EntityAnimal
{
    ...
    private static final DataParameter<Optional<UUID>> MATE_UNIQUE_ID   = EntityDataManager.<Optional<UUID>> createKey(EntityStallionDraftHorse.class,
            DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Boolean>        FIGHTING         = EntityDataManager.<Boolean> createKey(EntityStallionDraftHorse.class,
            DataSerializers.BOOLEAN);
    private static final Set<Item>                     TEMPTATION_ITEMS = Sets.newHashSet(new Item[] { Items.WHEAT, Items.APPLE });
    private static final DataParameter<Boolean>        WATERED          = EntityDataManager.<Boolean> createKey(EntityStallionDraftHorse.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean>        FED              = EntityDataManager.<Boolean> createKey(EntityStallionDraftHorse.class,
            DataSerializers.BOOLEAN);
    ...
    public EntityStallionDraftHorse(World world) {
        ...
        this.tasks.addTask(7, new EntityAITempt(this, 1.25D, false, EntityStallionDraftHorse.TEMPTATION_ITEMS));
    ...
    protected void entityInit() {
        super.entityInit();
        this.dataManager.register(EntityStallionDraftHorse.MATE_UNIQUE_ID, Optional.<UUID> absent());
        this.dataManager.register(EntityStallionDraftHorse.FIGHTING, Boolean.valueOf(false));
        this.dataManager.register(EntityStallionDraftHorse.FED, Boolean.valueOf(true));
        this.dataManager.register(EntityStallionDraftHorse.WATERED, Boolean.valueOf(true));
    }
    ...
    public UUID getMateUniqueId() {
        return (UUID) ((Optional) this.dataManager.get(EntityStallionDraftHorse.MATE_UNIQUE_ID)).orNull();
    }

    public void setMateUniqueId(@Nullable UUID uniqueId) {
        this.dataManager.set(EntityStallionDraftHorse.MATE_UNIQUE_ID, Optional.fromNullable(uniqueId));
    }

    public boolean getFighting() {
        return this.dataManager.get(EntityStallionDraftHorse.FIGHTING).booleanValue();
    }

    public void setFighting(boolean fighting) {
        if (fighting)
            this.dataManager.set(EntityStallionDraftHorse.FIGHTING, Boolean.valueOf(true));
        else
            this.dataManager.set(EntityStallionDraftHorse.FIGHTING, Boolean.valueOf(false));
    }

    public boolean getFed() {
        return this.dataManager.get(EntityStallionDraftHorse.FED).booleanValue();
    }

    public void setFed(boolean fed) {
        if (fed) {
            this.dataManager.set(EntityStallionDraftHorse.FED, Boolean.valueOf(true));
            this.fedTimer = AnimaniaConfig.careAndFeeding.feedTimer + this.rand.nextInt(100);
            this.setHealth(this.getHealth() + 1.0F);
        }
        else
            this.dataManager.set(EntityStallionDraftHorse.FED, Boolean.valueOf(false));
    }

    public boolean getWatered() {
        return this.dataManager.get(EntityStallionDraftHorse.WATERED).booleanValue();
    }

    public void setWatered(boolean watered) {
        if (watered) {
            this.dataManager.set(EntityStallionDraftHorse.WATERED, Boolean.valueOf(true));
            this.wateredTimer = AnimaniaConfig.careAndFeeding.waterTimer + this.rand.nextInt(100);
        }
        else
            this.dataManager.set(EntityStallionDraftHorse.WATERED, Boolean.valueOf(false));
    }
