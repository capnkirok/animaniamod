...
public class BlockInvisiblock extends BlockContainer
{
     ...
     * worldIn.getTileEntity(pos); if(te.getTrough() != null) {
     * if(!te.getTrough().itemHandler.getStackInSlot(0).isEmpty()) return
     * ItemHandlerHelper.calcRedstoneFromInventory(te.getTrough().itemHandler);
     *
     * if(te.getTrough().fluidHandler.getFluid() != null) { int fluid =
