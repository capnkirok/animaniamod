...
public class RemoveVanillaSpawns
{
    ...
    public void onEntitySpawn(WorldEvent.PotentialSpawns event) {
        ...
        for (int i = event.getList().size() - 1; i >= 0; i--)
            if (AnimaniaConfig.gameRules.replaceVanillaCows && event.getList().get(i).entityClass.getSimpleName().equals("EntityCow"))
                event.getList().remove(i);
            else if (AnimaniaConfig.gameRules.replaceVanillaChickens && event.getList().get(i).entityClass.getSimpleName().equals("EntityChicken"))
                event.getList().remove(i);
